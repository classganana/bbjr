...
import {
  BotMessage,
  MessageContainer,
} from "../studentaiAssistant/MessageContainerScreen";
...
export const BotScreen = () => {
  ...
  const setSubjectToLocal = async () => {
    try {
      if (subject && newUser) setNewUser(false);
  ...
  const deleteLastSuggestions = () => {
    ...
    lastMessages[messages.length - 1]
        ...
        if (res.data.statusCode == 200) {
          ...
          setMessages((prev: any) => {
            ...
            const index = updatedMessages.findIndex(
              (message) => message.source === "bot" && message.text === "typing"
            );
            if (index !== -1) {
  ...
  const submitFeedback = async (
    botFeedback: BotMessageFeedback | undefined
  ) => {
    ...
    const lastUserMessage = localMessages
      .reverse()
      .find((message: any) => message.source === "user");
    const req = {
      schoolId: "default",
      boardId: user?.board,
      subject: subject,
      ...
      msg: {
        feedbackMessage:
          botFeedback?.feedback == "negative" && userFeedback
            ? userFeedback
            : "liked",
  ...
  const getChatHistoryForFeedback = (botMsg: string | undefined) => {
    const index = messages.findIndex(
      (message: any) => message.source === "bot" && message.text === botMsg
    );
    if (index < 4) {
      return messages;
    } else {
          ...
          <View style={BotStyle.container}>
            <View style={BotStyle.header}>
              <View style={{ flexDirection: "row", gap: 5 }}>
                <TouchableOpacity
                  style={BotStyle.headerIcon}
                  onPress={onBackClick}
                >
                  <View
                    style={{
                      backgroundColor: "rgba(41, 71, 212, 0.07)",
                      borderRadius: 24,
                      padding: 5,
                      height: 40,
                      width: 40,
              ...
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <View style={BotStyle.selectedSubject}>
                  <TouchableOpacity
                    style={BotStyle.selectSubjectContainer}
                    onPress={() => {
                      setSubjectModal(true);
                    }}
                  >
                    {!subject ? (
                      <Text style={BotStyle.selectedSubjectText}>
                        Select Subject
                      </Text>
                    ) : (
                      <Text
                        numberOfLines={1}
                        ellipsizeMode="tail"
                        style={BotStyle.selectedSubjectText}
                      >
                        {subject}
                      </Text>
                    )}
            ...
            <View style={{ flex: 1 }}>
              {(messages && messages.length == 0) || subject == "" ? (
                ...
                <MessageContainer
                  messages={messages}
                  feedback={function (message: BotMessageFeedback): void {
            ...
            <View style={{ justifyContent: "flex-end", width: "100%" }}>
              <Aiinput
                ...
                onSendClick={(text: any) => pushMessageIntoQueue(text)}
